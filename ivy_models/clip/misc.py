...
def load_clip_state_dict(url: str):
    return torch.hub.load_state_dict_from_url(url, map_location="cpu").state_dict()


def get_processors(model):
    """Returns the text tokenizer and the image transform for a given model."""
...
def tokenize(
    ...
    truncate: bool
        Whether to truncate the text if its encoding is longer than the context length
