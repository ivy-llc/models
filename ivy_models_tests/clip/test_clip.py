...
@pytest.mark.parametrize("image_encoder", ["RN50", "ViT-B/32"])# ['RN50', 'RN101', 'RN50x4', 'RN50x16', 'RN50x64', 'ViT-B/32', 'ViT-B/16', 'ViT-L/14', 'ViT-L/14@336px'])
@pytest.mark.parametrize("image_name", ["cat.jpg"])
@pytest.mark.parametrize("text_input", [["a diagram", "a dog", "a cat"]])
@pytest.mark.parametrize("batch_shape", [[1]])
def test_all_clip_img_classification(device, f, fw, image_encoder, image_name, text_input, batch_shape):
    ...
    # Value test
    # using probs instead of logits because the original model is in fp16 and the ivy implementations is in float32ss    
